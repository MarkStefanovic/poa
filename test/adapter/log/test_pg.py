...
def log_fixture(pg_cursor_fixture: psycopg.Cursor) -> data.Log:
    cursor_provider = CursorProvider(cur=pg_cursor_fixture)
    return PgLog(cursor_provider=cursor_provider)


def test_error(pg_cursor_fixture: psycopg.Cursor, log_fixture: data.Log) -> None:
    log_fixture.error("Test")
    pg_cursor_fixture.execute("SELECT message FROM poa.error;")
    messages = [row["message"] for row in pg_cursor_fixture.fetchall()]
    assert messages == ["Test"]


def test_sync_failed(pg_cursor_fixture: psycopg.Cursor, log_fixture: data.Log) -> None:
    ...
    messages = [row["error_message"] for row in pg_cursor_fixture.fetchall()]
    assert messages == ["Test"]


def test_sync_skipped(pg_cursor_fixture: psycopg.Cursor, log_fixture: data.Log) -> None:
    pg_cursor_fixture.execute(
        """
        INSERT INTO poa.sync (sync_id, src_db_name, src_schema_name, src_table_name, incremental, ts) 
        OVERRIDING SYSTEM VALUE VALUES (1, 'src-db', 'src-schema', 'src-table', TRUE, now());
...
def test_sync_started(pg_cursor_fixture: psycopg.Cursor, log_fixture: data.Log) -> None:
    log_fixture.sync_started(
        src_db_name="src-db",
        src_schema_name="src-schema",
        src_table_name="src-table",
        incremental=True,
    )
    pg_cursor_fixture.execute(
        """
...
def test_sync_succeeded(pg_cursor_fixture: psycopg.Cursor, log_fixture: data.Log) -> None:
    pg_cursor_fixture.execute(
        """
        INSERT INTO poa.sync (sync_id, src_db_name, src_schema_name, src_table_name, incremental, ts) 
        OVERRIDING SYSTEM VALUE VALUES (1, 'src-db', 'src-schema', 'src-table', TRUE, now());
