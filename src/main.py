...
def sync(
    ...
    try:
        ...
        dst_connection_str = adapter.config.get_api(config_file=config_file, name=dst_ds_name)
        ...
        try:
            ...
            dst_api = adapter.config.get_api(config_file=config_file, name=dst_ds_name)
            dst_connection_str = adapter.config.get_connection_str(config_file=config_file, name=dst_ds_name)
            ...
            with src_cursor_provider.open() as src_cur, dst_cursor_provider.open() as dst_cur:
                ...
                if result.status == "succeeded":
                    ...
                    log.sync_failed(sync_id=sync_id, reason=result.error_message)
                elif result.status == "skipped":
                    log.sync_skipped(sync_id=sync_id, reason=result.skip_reason)
                else:
...
def _init_loguru_logger(*, log_folder: pathlib.Path = adapter.fs.get_log_folder(), filename_prefix: str = ""):
